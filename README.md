<!-- Banner -->
![Banner](https://your-image-link.com/banner.png)

<br>

# 👋 Hi, I'm Prabath Wanasinghe  
### 🚀 Robotics | Web Developer | Tech Enthusiast

I'm a student developer passionate about building robots, automating things, and creating real-world tech solutions.

---

## 🔥 About Me
- 🤖 Robotics & embedded systems enthusiast  
- 💡 I love working with ESP32, Arduino & sensors  
- 🌱 Currently learning: Web Development, AI tools  
- 🎯 Goal: Build fast, smart competition-ready robots  
- 🧪 Always experimenting with new ideas  

---

## 🛠️ Tech Stack
<!-- Badges -->
![C++](https://img.shields.io/badge/C++-00599C?logo=c%2B%2B&logoColor=white)
![Arduino](https://img.shields.io/badge/Arduino-00878F?logo=arduino&logoColor=white)
![ESP32](https://img.shields.io/badge/ESP32-black?logo=espressif&logoColor=white)
![HTML](https://img.shields.io/badge/HTML-E34F26?logo=html5&logoColor=white)
![CSS](https://img.shields.io/badge/CSS-1572B6?logo=css3&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?logo=javascript&logoColor=black)

---

## ✨ Typing Animation
[![Typing SVG](https://readme-typing-svg.herokuapp.com?size=28&lines=Robotics+Developer;ESP32+Programmer;Web+Developer;Tech+Explorer)](https://git.io/typing-svg)

---

## 🚀 Featured Projects

### 🔹 Line Following Robot (ESP32)
A fast and optimized line follower built for competition performance.

### 🔹 Maze Solving Robot
Autonomous maze navigation using IR sensors + custom algorithms.

### 🔹 3D Portfolio Website
A modern 3D portfolio built with HTML, CSS, and JS.

---

## 📊 GitHub Stats  
![GitHub Stats](https://github-readme-stats.vercel.app/api?username=YOUR_USERNAME&show_icons=true&theme=default)

### 🔹 Top Languages  
![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=YOUR_USERNAME&layout=compact)

---

## 🌐 Connect With Me
[![Instagram](https://img.shields.io/badge/Instagram-E4405F?logo=instagram&logoColor=white)](https://instagram.com/yourname)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?logo=linkedin&logoColor=white)](https://linkedin.com/in/yourname)

---


## 📌 Quote
> “The best way to predict the future is to build it.”

